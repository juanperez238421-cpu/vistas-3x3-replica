/* Vistas 3x3 · Senior trace workbench v2 */
...
function buildGuideRow() {
  ...
  [["Retícula", "3×3"], ["Ajuste", "½ módulo"], ["Segmento", "clic inicial + clic final"]].forEach(([name, value]) => {
...
function setupToolbarAndExpansion() {
  ...
  /* Important: new rows are siblings; never place sourceRow inside its own replacement tree. */
  const drawingRow = document.createElement("div");
  drawingRow.className = "tool-row trace-tool-grid trace-tool-grid-drawing";
  drawingRow.append(...drawingButtons);
  ...
  toolbar.append(buildToolbarGroup("Dibujo", drawingRow), buildToolbarGroup("Documento", documentRow));
  ...
  function setExpanded(expanded) {
    ...
    expandButton.setAttribute("aria-label", expanded ? "Reducir panel de trazo" : "Ampliar panel de trazo");
  ...
  expandButton.addEventListener("click", () => setExpanded(!boardPanel.classList.contains("is-expanded")));
  ...
  return { boardPanel, expandButton, setExpanded };
}
...
function drawSeniorGrid(ctx, rect) {
  ...
  [1, 3, 5].forEach((step) => {
    ...
    ctx.beginPath(); ctx.moveTo(rect.x + offset, rect.y); ctx.lineTo(rect.x + offset, rect.y + rect.size); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(rect.x, rect.y + offset); ctx.lineTo(rect.x + rect.size, rect.y + offset); ctx.stroke();
  });
  ...
  for (let row = 0; row < 3; row += 1) {
    for (let col = 0; col < 3; col += 1) {
      ctx.beginPath();
      ctx.arc(rect.x + major * (col + 0.5), rect.y + major * (row + 0.5), clamp(rect.size * 0.006, 1, 2.3), 0, Math.PI * 2);
      ctx.fill();
    }
  }
  ctx.restore();
...
function setupSeniorGridOverlay() {
  ...
  if (!paperShell || !paperStack || !solutionCanvas) return null;
