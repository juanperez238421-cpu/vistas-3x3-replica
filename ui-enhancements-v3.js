/* Progressive, non-destructive UI semantics for Vistas 3x3. */
...
function applyStaticSemantics() {
  q(".sidebar-tabs")?.setAttribute("role", "tablist");
  q(".sidebar-tabs")?.setAttribute("aria-label", "Secciones de aprendizaje");
  q(".difficulty-tabs")?.setAttribute("role", "group");
  q(".difficulty-tabs")?.setAttribute("aria-label", "Nivel de dificultad");
  q("#piece-meta")?.setAttribute("aria-live", "polite");
  q("#current-view-label")?.setAttribute("aria-live", "polite");
...
function bindKeyboardShortcuts() {
  window.addEventListener("keydown", (event) => {
    ...
    const map = { "1": "isometric", "2": "front", "3": "top", "4": "right", "5": "left" };
